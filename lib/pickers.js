...
function filterProposedDropdown(query) {
  ...
  // Even with zero matches we still show the dropdown so the "+ Create custom"
  // affordance is reachable. If the user typed something, prefill the new
  // person's name with it.
  ...
  // Always offer to create a custom person at the bottom. Prefills name with
  // the current query if any.
  const ccLabel = q ? ('+ Create custom person "' + q + '"') : '+ Create custom person…';
  html += '<div class="proposed-dd-section">External</div>' +
    '<div class="proposed-dd-create" data-create-prefill="' + escapeHtml(q) + '">' +
      '<span class="av-mini" style="background:#f0efeb;color:#1a1a1a">+</span>' +
      '<div class="proposed-dd-meta"><div class="proposed-dd-name">' + escapeHtml(ccLabel) + '</div>' +
      '<div class="proposed-dd-email">Not in Bookline Slack — create an external person</div></div>' +
    '</div>';
  dd.innerHTML = html;
  dd.style.display = '';
  ...
  // Wire the "+ Create custom person" affordance.
  const createRow = dd.querySelector('.proposed-dd-create');
  if (createRow) {
    createRow.addEventListener('mousedown', e => e.preventDefault()); // keep input focus
    createRow.addEventListener('click', e => {
      e.stopPropagation();
      const prefill = createRow.dataset.createPrefill || '';
      if (typeof openAddMember === 'function') openAddMember({ mode: 'custom', prefillName: prefill });
    });
  }
}
...
function _respPickerRenderList(query) {
  ...
  // Always show "+ Create custom person" affordance at the bottom.
  const ccLabel = q ? ('+ Create custom person "' + q + '"') : '+ Create custom person…';
  html += '<div class="resp-picker-section-label">External</div>' +
    '<div class="resp-picker-item resp-picker-create" data-create-prefill="' + escapeHtml(q) + '">' +
      '<span class="av-mini" style="width:32px;height:32px;background:#f0efeb;color:#1a1a1a;display:inline-flex;align-items:center;justify-content:center;border-radius:999px;font-weight:700">+</span>' +
      '<div class="resp-picker-item-meta">' +
        '<div class="resp-picker-item-name">' + escapeHtml(ccLabel) + '</div>' +
        '<div class="resp-picker-item-email">Not in Bookline Slack — create external</div>' +
      '</div>' +
    '</div>';
  list.innerHTML = html;
  _respPickerActiveIndex = -1;
  list.querySelectorAll('.resp-picker-item:not(.resp-picker-create)').forEach(it => {
  ...
  const createRow = list.querySelector('.resp-picker-create');
  if (createRow) {
    createRow.addEventListener('mousedown', e => e.preventDefault());
...
const COLORED_PICKLIST_OPTIONS = {
  ...
  f_priority: [
    ...
    { value: 'Low',      label: 'Low',      bg: '#e0f2fe', fg: '#0369a1', border: '#7dd3fc' },
    { value: 'Very Low', label: 'Very Low', bg: '#c7d2fe', fg: '#1e3a8a', border: '#818cf8' },
  ],
  f_type: [
    { value: '',               label: '(empty)',        bg: '#ffffff', fg: '#6b6b6b', border: '#d8d6d1' },
