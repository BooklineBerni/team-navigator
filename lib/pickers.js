...
const COLORED_PICKLIST_OPTIONS = {
  ...
  f_priority: [
    ...
    { value: 'Alta',      label: 'Alta',      bg: '#fef0c7', fg: '#b54708', border: '#fde68a' },
    { value: 'Media',     label: 'Media',     bg: '#d1fadf', fg: '#027a48', border: '#a7f3d0' },
    { value: 'Baja',      label: 'Baja',      bg: '#eaecf0', fg: '#475467', border: '#d8d6d1' },
    { value: 'Muy Baja',  label: 'Muy Baja',  bg: '#f2f4f7', fg: '#98a2b3', border: '#e2e4e7' },
  ],
  f_type: [
    { value: '',               label: '(empty)',        bg: '#ffffff', fg: '#6b6b6b', border: '#d8d6d1' },
...
function _groupPickerRenderChip(currentGroupId) {
  ...
  if (!g) {
    chip.removeAttribute('data-status');
    chip.innerHTML = '<span class="gp-empty">(missing group)</span><button type="button" class="gp-clear" title="Clear">×</button><span class="gp-caret">▾</span>';
  } else {
    ...
    chip.innerHTML =
      ...
      '<span class="gp-name">' + escapeHtml(g.subject || '(unnamed)') + '</span>' +
      '<button type="button" class="gp-clear" title="Clear">×</button>' +
      '<span class="gp-caret">▾</span>';
  }
  const clr = chip.querySelector('.gp-clear');
...
(function wireGroupPicker(){
  ...
  // The dropdown stays anchored to the chip's current viewport position.
  ...
  inp.addEventListener('input', () => { _groupPickerRenderList(inp.value); _bnPositionGroupDropdown(); });
  inp.addEventListener('keydown', e => { if (e.key === 'Escape') { dd.style.display = 'none'; chip.focus(); } });
  document.addEventListener('click', e => {
    const box = document.getElementById('f_groupBox');
    if (!box) return;
    if (!box.contains(e.target)) dd.style.display = 'none';
  });
})();
