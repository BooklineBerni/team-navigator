...
function renderRoadmapsInModal(t) {
  ...
  if (assigned.length === 0) {
    ...
    cont.innerHTML = '<div class="rm-modal-list">' + assigned.map(r => {
      return '<div class="rm-modal-row bn-ro-allow" data-rmid="' + escapeHtml(r.id) + '">' +
        '<button type="button" class="rm-modal-link bn-ro-allow" data-rmid="' + escapeHtml(r.id) + '" title="Open this roadmap">' +
        ...
        list.querySelectorAll('.rm-add-item').forEach(it => {
          ...
          it.addEventListener('click', () => {
            ...
            if (!rm.tasks.some(e => e.taskId === t.id)) {
              // Read LIVE dates from the form inputs first — for a brand-new
              // task the user just typed them and they haven't been written
              // back to t.startDate/t.endDate yet (that happens on Save).
              // Falling back to t.* covers existing tasks being edited.
              const _liveStartEl = document.getElementById('f_taskStart');
              const _liveEndEl   = document.getElementById('f_taskEnd');
              const _liveStart = (_liveStartEl && _liveStartEl.value) || t.startDate || '';
              const _liveEnd   = (_liveEndEl   && _liveEndEl.value)   || t.endDate   || '';
              const newEntry = { taskId: t.id };
              if (_liveStart) newEntry.startDate = _liveStart;
              if (_liveEnd)   newEntry.endDate   = _liveEnd;
              // ALSO mirror onto the task object so the calendar reads them
              // immediately, even before Save commits.
              if (_liveStart) t.startDate = _liveStart;
              if (_liveEnd)   t.endDate   = _liveEnd;
