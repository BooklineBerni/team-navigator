...
// True ONLY when the active permission is the member role and we're not in a preview.
// Used to unlock the one editable affordance members have: changing the slackStatus on
// tasks they're the responsible of.
function bnIsMember() {
  return (typeof bnUserPermission !== 'undefined' && bnUserPermission === 'member') &&
         !(typeof bnPreviewAsEmail !== 'undefined' && !!bnPreviewAsEmail);
}
...
async function bnSyncPullFromCloud() {
  ...
  try {
    ...
    try {
      ...
      // Distinct class for members so CSS can RE-ENABLE the slackStatus picker on
      // their own tasks. Set only outside of preview — when an admin previews as a
      // member, we want the strictest read-only behaviour.
      document.body.classList.toggle('bn-member',
        (bnUserPermission === 'member') && !bnPreviewAsEmail);
    } catch (_) {}
