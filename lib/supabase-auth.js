...
async function bnSyncPullFromCloud() {
  ...
  try {
    ...
    // Update read-only banner + body class. The class lets CSS visually disable every
    // input/button that could mutate the store (see app.css: body.bn-read-only).
    // When previewing-as, the sidebar shows a dedicated card (avatar + email + permission)
    // instead of the top banner — so we suppress the banner in that case to avoid noise.
    ...
    if (banner) {
      if (window.__bnReadOnly && !bnPreviewAsEmail) {
        ...
        if (msg) msg.textContent = "Read-only view — your role is restricted_view. You can browse but not modify.";
      } else {
