...
document.addEventListener('DOMContentLoaded', () => {
  ...
  bnAuthBoot();
  // Defense-in-depth: even if the CSS doesn't catch every editor button, the JS layer
  // refuses to persist any mutation when in read-only mode. We wrap `saveStore` so any
  // in-memory STORE change made by a leaked editor action stays local and disappears on
  // refresh. Wrapping happens after DOMContentLoaded so the inline script has defined
  // saveStore by now (it's a `function` declaration on the global script scope).
  try {
