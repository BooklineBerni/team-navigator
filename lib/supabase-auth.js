...
function bnAuthShowGate(show, options) {
  ...
  if (show) {
    ...
    if (mode === 'waiting') {
      if (title) title.textContent = 'Waiting for permissions';
      if (subtitle) {
        const who = options.email ? (' (' + options.email + ')') : '';
        subtitle.innerHTML = 'Your account' + who + ' is signed in, but the admin has not granted you permissions to view tasks yet.<br><br>Ask <strong>bernat@bookline.ai</strong> to grant you access from the <em>Admin</em> section.';
      }
    ...
    if (err) {
      if (options.errorMsg) { err.textContent = options.errorMsg; err.style.display = ''; } else err.style.display = 'none';
    }
    ...
    // Lift the anti-flash overlay (was set pre-paint by the head script when a hash
    // carried an OAuth callback). Now that the app is ready, let the body paint.
    try { document.documentElement.removeAttribute('data-bn-oauth-callback'); } catch (_) {}
  }
}
...
function bnHydrateSessionFromHashSync() {
  try {
    ...
    const ref = m ? m[1] : '';
    try { localStorage.setItem('sb-' + ref + '-auth-token', JSON.stringify(session)); } catch(_) {}
    // Wipe tokens out of the URL so they don't linger in history.
    try { history.replaceState(null, '', location.pathname + location.search); } catch(_) {}
    bnSupabaseSession = session;
    bnSupabaseUser = user;
    console.info('[BN] Hydrated session synchronously from hash · user:', user.email);
    return user;
  } catch (e) { console.warn('[BN] sync hydrate exception:', e && e.message); return null; }
}
...
document.addEventListener('DOMContentLoaded', () => {
  ...
  if (BN_AUTH_REQUIRED) {
    if (bnHasCachedSession) {
      ...
      document.body.classList.remove('bn-locked');
    } else if (bnHashHasOAuth || bnSupabaseUser) {
