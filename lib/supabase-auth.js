...
function bnAuthShowGate(show, options) {
  ...
  if (show) {
    ...
    if (err) {
      // Always show a diagnostic line on the sign-in screen so we (and the user) can see
      // exactly what state the page is in — useful for debugging the re-auth loop without
      // opening DevTools or asking the user to copy-paste console output.
      ...
      if (options.errorMsg) {
        ...
        let reason;
        if (oauthErr) {
          reason = 'OAuth provider error: ' + oauthErr.error + (oauthErr.description ? ' — ' + oauthErr.description : '');
        } else if (cameFromCallback) {
          reason = hydrateErr === 'localstorage-blocked'
          ...
          // Fresh sign-in (no callback). Show a tiny build/state marker so we can verify the
          // user is on the latest deploy AND see their starting state.
          const ls = (function() {
            try {
              const m = (BN_SUPABASE_URL || '').match(/^https?:\/\/([^.]+)\./);
              const ref = m ? m[1] : '';
              return !!localStorage.getItem('sb-' + ref + '-auth-token');
            } catch (e) { return 'blocked:' + (e && e.message || 'err'); }
          })();
          reason = '[build:' + BN_AUTH_BUILD + '] cached-session=' + ls + ' · ' +
                   'oauth-callback=false · href=' + (snap.href ? snap.href.replace(/^https?:\/\/[^/]+/, '') : '?');
        }
        err.textContent = reason;
        err.style.color = (oauthErr || cameFromCallback) ? '#fca5a5' : '#9a9a9a';
        err.style.fontSize = '12px';
        err.style.display = '';
        if (oauthErr || cameFromCallback) {
