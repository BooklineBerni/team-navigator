...
// ---------------------------------------------------------------------------
// Auth gate + cloud sync layer for Team Navigator.
//
...
// Build tag — bump this whenever auth code changes so we can verify deployed clients are
// on the latest version (shown in the auth gate's diagnostic line).
const BN_AUTH_BUILD = "2026-05-18-c";
let bnSupabase = null;
try {
...
// Centralised "user is restricted" check. Returns true for actual restricted_view users,
// members, AND for admins previewing as another user (so the UI behaves identically in
// all cases — locked down by default).
//   • restricted_view → read-only everywhere
//   • member          → read-only everywhere EXCEPT the status field on tasks they own;
//                        bnIsMember() below identifies that special case
//   • preview-as      → admin viewing as another user; treated like the target
function bnIsRestrictedView() {
  return (typeof bnUserPermission !== 'undefined' &&
            (bnUserPermission === 'restricted_view' || bnUserPermission === 'member')) ||
         (typeof bnPreviewAsEmail !== 'undefined' && !!bnPreviewAsEmail);
}
// True when the EFFECTIVE permission is the member role. Used to unlock the
// one editable affordance members have: changing the slackStatus on tasks
// they're the responsible of.
//   • Real member sign-in: bnUserPermission === 'member' and not in preview.
//   • Admin previewing as a member: bnPreviewAsInfo.permission === 'member'.
//     We honour this so the admin can verify the member UI from preview-as
//     instead of having to sign in as the user. (Server-side writes from a
//     previewing admin will still fail the member_set_task_status check, but
//     the UI behaviour matches what the member would see.)
function bnIsMember() {
  if (typeof bnUserPermission !== 'undefined' && bnUserPermission === 'member' &&
      !(typeof bnPreviewAsEmail !== 'undefined' && !!bnPreviewAsEmail)) return true;
  if (typeof bnPreviewAsEmail !== 'undefined' && bnPreviewAsEmail &&
      typeof bnPreviewAsInfo !== 'undefined' && bnPreviewAsInfo &&
      bnPreviewAsInfo.permission === 'member') return true;
  return false;
}
function bnAuthShowGate(show, options) {
  ...
  if (show) {
    ...
    if (mode === 'waiting') {
      ...
      if (title) title.textContent = 'Team Navigator';
    ...
    if (err) {
      // Show diagnostic info ONLY when something has actually gone wrong — either we were
      // passed an explicit error, OR the user just came back from an OAuth callback but
      // ended up at the sign-in form anyway (meaning hydration failed). For a clean fresh
      // sign-in, no message — the form is self-explanatory.
      ...
      if (options.errorMsg) {
        ...
        err.style.display = '';
      } else if (mode === 'signin' && (oauthErr || cameFromCallback)) {
        const reason = oauthErr
          ...
          ? 'Your browser blocked saving the session (private/incognito mode or storage disabled).'
          : hydrateErr === 'jwt-decode'
          ? 'The auth token returned by Google could not be decoded.'
          : hydrateErr === 'pkce-timeout'
          ? 'The Supabase code exchange timed out. Your browser may be blocking storage for supabase.co. Try disabling tracking protection or use a different browser.'
          : hydrateErr === 'no-hash'
          ? 'Returned from Google but no tokens in the URL. Supabase may have stripped them.'
          : 'Sign-in did not complete. (hydrate=' + (hydrateErr || 'unknown') + ', hash=' + !!snap.hasHashToken + ', code=' + !!snap.hasQueryCode + ')';
        err.textContent = reason;
        err.style.color = '#fca5a5';
        err.style.fontSize = '13px';
        err.style.display = '';
        console.warn('[BN] Sign-in form shown after callback. Diagnostic:', { snap, hydrateErr });
      } else {
...
async function bnSyncPullFromCloud() {
  ...
  try {
    ...
    // Decide read-only by ROLE, not just by the cloud response.
    //   • admin (no preview)  → editor, never read-only
    //   • admin (preview-as)  → read-only (viewing as another user)
    //   • restricted_view     → read-only
    //   • anything else       → read-only by default
    // The `filtered` flag from the RPC is informational only.
    if (bnUserPermission === 'admin' && !bnPreviewAsEmail) {
      window.__bnReadOnly = false;
    } else {
      window.__bnReadOnly = true;
    }
    if (!data || Object.keys(data).length === 0) {
      ...
      STORE = data;
      // CRITICAL: only persist this STORE to localStorage if it's the admin's OWN
      // full store. A filtered preview must never overwrite the admin's cached data —
      // doing that once caused a corruption incident (admin store replaced with 77
      // tasks of the previewed user). bookline-team-panel-v2 stays untouched while
      // previewing, so exiting preview cleanly restores the admin's data.
      const isOwnAdminStore = (bnUserPermission === 'admin' && !bnPreviewAsEmail && !filtered);
      if (isOwnAdminStore) {
        try { saveStore(STORE); } catch(_) {}
      } else {
        console.info('[BN] saveStore SKIPPED — pulled store is a filtered/preview view; admin localStorage left intact.');
      }
      // CRITICAL: rebuildTeam() refreshes the global TEAM array from
      // DEFAULT_TEAM + STORE.customMembers. Without this, exiting preview-as
      // and pulling the admin's full store back leaves TEAM stale — boot ran
      // rebuildTeam() on an empty STORE so customMembers / people-groups
      // weren't in TEAM, and the People page didn't render group cards even
      // though STORE.customMembers had them. Calling it here after the pull
      // closes the loop.
      try { if (typeof rebuildTeam === 'function') rebuildTeam(); } catch (_) {}
      // Mark that we've synced from the cloud. Used by renderRequestsPage to differentiate
      // "STORE.requestActions is empty because cloud sync hasn't happened" from
      // "STORE.requestActions is empty because nothing was dismissed". Without this flag,
      // exiting preview-as (which reloads the page) would briefly render every cached
      // Slack request as "pending" because the local STORE.requestActions starts at {}.
      try { window.__bnCloudSyncedAt = Date.now(); } catch (_) {}
      // Refresh the requests sidebar badge — before cloud sync we hide it because
      // bnPendingRequests would have shown ALL cached requests as pending (no
      // requestActions yet). Now that we have the actions, recompute.
      try { if (typeof bnUpdateRequestsBadge === 'function') bnUpdateRequestsBadge(); } catch (_) {}
      if (typeof render === 'function') { try { render(); } catch(_) {} }
      console.info('[BN] Pulled cloud store · tasks:', (STORE.tasks||[]).length, '· filtered:', filtered, '· readOnly:', window.__bnReadOnly, '· persisted:', isOwnAdminStore);
    }
    // The yellow "Read-only view" banner used to live here; it was removed because
    // the sidebar role pill + the modal "View only" pill already convey the state.
    // The body class below remains the source of truth for the CSS lockdown.
    try {
      document.body.classList.toggle('bn-read-only', !!window.__bnReadOnly);
      // Distinct class for members so CSS can RE-ENABLE the slackStatus picker
      // on their own tasks. Applies to BOTH:
      //   – real member sign-in (bnUserPermission === 'member'), and
      //   – admin previewing-as a member (so preview faithfully matches what
      //     the member would see). bnIsMember() returns true for both cases.
      document.body.classList.toggle('bn-member', typeof bnIsMember === 'function' && bnIsMember());
    } catch (_) {}
...
async function bnSyncPushToCloud(options) {
  ...
  // We check BOTH __bnReadOnly and bnPreviewAsEmail — either being truthy means the
  // in-memory STORE is not the admin's authoritative data and must not be pushed.
  if (window.__bnReadOnly || bnPreviewAsEmail) {
    console.info('[BN] push BLOCKED — STORE is a filtered/preview copy (readOnly=' + window.__bnReadOnly + ', preview=' + bnPreviewAsEmail + ').');
    return;
  }
  ...
  const doPush = async () => {
    try {
      // SAFEGUARD: preserve cloud's driveFiles if local has none. This guards against the
      // "files keep disappearing" failure: a tab that loaded before files were imported, or
      // one whose pull failed, can otherwise stomp an empty driveFiles[] over a populated
      // cloud row. We fetch the cloud's current driveFiles count and refuse to overwrite
      // a populated cloud with an empty local array.
      try {
        const localFiles = Array.isArray(STORE.driveFiles) ? STORE.driveFiles : [];
        if (localFiles.length === 0) {
          const { data: cur } = await bnSupabase
            .from('user_stores')
            .select('store_data')
            .eq('user_id', bnSupabaseUser.id)
            .maybeSingle();
          const cloudFiles = (cur && cur.store_data && Array.isArray(cur.store_data.driveFiles))
            ? cur.store_data.driveFiles : [];
          if (cloudFiles.length > 0) {
            // Adopt cloud's files into both STORE and the about-to-be-pushed payload.
            STORE.driveFiles = cloudFiles.slice();
            try { if (typeof saveStore === 'function') saveStore(STORE); } catch (_) {}
            console.info('[BN] push: preserved ' + cloudFiles.length + ' driveFiles from cloud (local was empty).');
          }
        }
      } catch (e) { console.warn('[BN] push: driveFiles preserve check failed:', e && e.message); }
...
// ===== Periodic auto-refresh from Supabase =====
// Long-running sessions used to drift: if someone left the tab open all day
// they wouldn't see changes made by other admins until they hit reload. The
// timer below polls the cloud at a fixed cadence and refreshes the in-memory
// STORE if the user isn't actively doing something.
//
// Conservative triggers (must ALL be true for an auto-pull to fire):
//   - tab is visible
//   - no task modal is open (editingId is falsy)
//   - no other modal overlay is open (.modal-bg.show / .person-modal-bg.show)
//   - the active element isn't a text-entry field (input, textarea, select, contenteditable)
//   - at least BN_AUTO_PULL_MIN_GAP_MS has elapsed since the last successful pull
//
// The timer is started exactly once per session after the auth gate resolves
// the user is allowed to read the cloud store (admin OR restricted_view OR
// member). For 'admin in preview-as' we still poll — pulling the previewed
// view stays consistent for the previewer.
const BN_AUTO_PULL_INTERVAL_MS = 120000;   // probe every 2 minutes
const BN_AUTO_PULL_MIN_GAP_MS  = 60000;    // never two pulls within 1 minute
let bnAutoPullTimer = null;
let bnLastPullAt = 0;
function bnUserIsActivelyEditing() {
  // Task modal open?
  try {
    if (typeof editingId !== 'undefined' && editingId) return true;
  } catch (_) { /* editingId is a let inside inline — may not be visible from here */ }
  // Any visible modal overlay?
  const visibleModals = document.querySelectorAll(
    '.modal-bg.show, .person-modal-bg.show, .bn-modal.show, .bn-roadmap-modal.show'
  );
...
async function bnMaybeAutoPull() {
  try {
    if (!bnSupabase || !bnSupabaseUser) return;
    if (document.hidden) return;
    if (Date.now() - bnLastPullAt < BN_AUTO_PULL_MIN_GAP_MS) return;
    if (bnUserIsActivelyEditing()) return;
    // Skip if a push is pending — the debounce will fire shortly and we don't
    // want to overwrite the about-to-be-pushed local state with cloud's older
    // copy. (Push then pull on the next tick handles this naturally.)
    if (bnSyncPushTimer) return;
    bnLastPullAt = Date.now();
    await bnSyncPullFromCloud();
  } catch (e) {
    console.warn('[BN] auto-pull error:', e && e.message);
  }
}
function bnStartAutoPullLoop() {
  if (bnAutoPullTimer) return;   // already running
  bnAutoPullTimer = setInterval(bnMaybeAutoPull, BN_AUTO_PULL_INTERVAL_MS);
  // Also pull when the tab becomes visible again after being hidden — catches
  // the case where someone returns to a long-idle tab.
  document.addEventListener('visibilitychange', () => {
    if (!document.hidden) {
      // tiny delay so window state settles before we pull
      setTimeout(bnMaybeAutoPull, 500);
    }
  });
  console.info('[BN] auto-pull loop started (every ' + (BN_AUTO_PULL_INTERVAL_MS / 1000) + 's).');
}
window.bnStartAutoPullLoop = bnStartAutoPullLoop;
window.bnMaybeAutoPull = bnMaybeAutoPull;
...
async function bnAuthResolveGate() {
  ...
  if (perm === 'admin' || perm === 'restricted_view' || perm === 'member') {
    ...
    bnSyncPullFromCloud()
      .then(() => { bnLastPullAt = Date.now(); })
      .catch(e => console.warn('[BN] background pull error:', e && e.message));
    // Kick off the periodic auto-refresh so long-lived sessions stay in sync.
    try { bnStartAutoPullLoop(); } catch (_) {}
  } else {
...
document.addEventListener('DOMContentLoaded', () => {
  ...
  bnAuthBoot();
  // Defense-in-depth #1: wrap saveStore so any in-memory STORE change made by a leaked
  // editor action stays local and disappears on refresh.
  // Exception: when a MEMBER is saving a status change on their own task, we let the
  // write through (the member click handler sets window.__bnMemberStatusSave for the
  // duration of the call). The server-side RPC enforces the rest of the policy.
  try {
    if (typeof saveStore === 'function' && !saveStore.__bnROWrapped) {
      const _origSaveStore = saveStore;
      saveStore = function() {
        if (window.__bnReadOnly && !window.__bnMemberStatusSave) {
          console.info('[BN] saveStore skipped — read-only mode.');
          return;
        }
        return _origSaveStore.apply(this, arguments);
      };
      saveStore.__bnROWrapped = true;
    }
  } catch (_) {}
  // Defense-in-depth #2: block drag operations on calendar pills (the only obvious
  // "drag-to-mutate" affordance left after the CSS rules). Everything else is allowed
  // to navigate freely — the saveStore wrap above ensures no mutation can ever be
  // persisted to localStorage or the cloud.
  ['dragstart', 'drop'].forEach(evt => {
    document.addEventListener(evt, e => {
      if (!window.__bnReadOnly) return;
      e.preventDefault();
