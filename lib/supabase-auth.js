...
document.addEventListener('DOMContentLoaded', () => {
  ...
  // Defense-in-depth #2: capture-phase event interceptor. If anything mutational sneaks
  // past the CSS, this catches it. Allowlist: sidebar nav, search, filter pills, sign-out,
  // banner, modal close/cancel buttons, task-card clicks (which open the read-only modal),
  // view toggles, expand/collapse. Everything else is blocked.
  function bnROIsAllowed(t) {
    if (!t || !t.closest) return false;
    return !!(
      t.closest('.sidebar') ||
      t.closest('#bnReadOnlyBanner') ||
      t.closest('#bnAuthSignOutBtn') ||
      t.closest('#searchInput') ||
      t.closest('.search-input') ||
      t.closest('.filter-pill') ||
      t.closest('.filters') ||
      t.closest('[data-bn-ro-allow]') ||
      t.closest('[data-view-mode]') ||
      t.closest('[class*="view-toggle"]') ||
      t.closest('[class*="view-mode"]') ||
      t.closest('[class*="expand"]') ||
      t.closest('[class*="collapse"]') ||
      t.closest('[class*="chevron"]') ||
      t.closest('.task-card') ||
      t.closest('.task-row') ||
      // Modal-close zone
      (t.closest('.modal-bg') && (
        t === t.closest('.modal-bg') ||                // background click
        t.closest('.modal-close') ||
        t.closest('.close-btn') ||
        t.closest('[class*="close"]') ||
        t.closest('[id*="close" i]') ||
        t.closest('[class*="cancel"]') ||
        t.closest('[id*="cancel" i]')
      ))
    );
  }
  function bnROBlocker(e) {
    if (!window.__bnReadOnly) return;
    if (bnROIsAllowed(e.target)) return;
    e.preventDefault();
    e.stopImmediatePropagation();
  }
  // Capture-phase so we run BEFORE any addEventListener / onclick handler.
  ['click', 'mousedown', 'pointerdown', 'dblclick', 'dragstart', 'drop', 'paste'].forEach(evt => {
    document.addEventListener(evt, bnROBlocker, true);
  });
  // For keyboard: block keys that would mutate (Enter on a non-allowlisted form field, etc.)
  document.addEventListener('keydown', e => {
    ...
    if (bnROIsAllowed(e.target)) return;
    // Block typing into anything non-allowlisted (modal inputs, in-place editors, etc.)
