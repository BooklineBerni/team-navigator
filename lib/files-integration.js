...
// Tracks the in-modal selection of linked tasks / roadmaps so the user can
// add and remove links before clicking Save. On Save we persist these onto
// the file record as taskIds[] / roadmapIds[]. Reset on every openFileModal.
let _ffLinkedTaskIds = [];
let _ffLinkedRoadmapIds = [];
...
function openFileModal(id) {
  ...
  // Hydrate the link-pickers from the file record (defaults to empty arrays).
  _ffLinkedTaskIds = Array.isArray(f.taskIds) ? f.taskIds.slice() : [];
  _ffLinkedRoadmapIds = Array.isArray(f.roadmapIds) ? f.roadmapIds.slice() : [];
  _ffRenderLinkChips();
  // Reset the search inputs and hide their result dropdowns.
  const tInp = document.getElementById('ff_linkTasksSearch');
  const rInp = document.getElementById('ff_linkRmSearch');
  if (tInp) tInp.value = '';
  if (rInp) rInp.value = '';
  const tRes = document.getElementById('ff_linkTasksResults');
  const rRes = document.getElementById('ff_linkRmResults');
...
// Render the chip rows for current linked tasks/roadmaps. Each chip has an ×
// button that removes the link from the local _ff*Ids array (persisted on Save).
function _ffRenderLinkChips() {
  const tCont = document.getElementById('ff_linkTasksChips');
  const rCont = document.getElementById('ff_linkRmChips');
  if (tCont) {
    if (!_ffLinkedTaskIds.length) {
      tCont.innerHTML = '<span style="color:#9a9a9a; font-size:11.5px">No tasks linked</span>';
    } else {
      tCont.innerHTML = _ffLinkedTaskIds.map(tid => {
        const tk = (STORE.tasks || []).find(x => x.id === tid);
        const subj = tk ? (tk.subject || '(no title)') : '(missing task)';
        return '<span class="ff-link-chip" data-tid="' + escapeHtml(tid) + '" style="display:inline-flex; align-items:center; gap:4px; padding:2px 6px 2px 8px; background:#eef2ff; color:#1e3a8a; border-radius:12px; font-size:11.5px">' +
          escapeHtml(subj.slice(0, 50)) + (subj.length > 50 ? '…' : '') +
          '<button type="button" class="ff-link-x" data-rm-tid="' + escapeHtml(tid) + '" style="background:transparent; border:none; cursor:pointer; color:#1e3a8a; font-size:14px; padding:0 2px; line-height:1">×</button>' +
        '</span>';
      }).join('');
      tCont.querySelectorAll('.ff-link-x').forEach(btn => {
        btn.addEventListener('click', () => {
          _ffLinkedTaskIds = _ffLinkedTaskIds.filter(x => x !== btn.dataset.rmTid);
          _ffRenderLinkChips();
        });
      });
    }
  ...
  if (rCont) {
    if (!_ffLinkedRoadmapIds.length) {
      rCont.innerHTML = '<span style="color:#9a9a9a; font-size:11.5px">No roadmaps linked</span>';
    } else {
      rCont.innerHTML = _ffLinkedRoadmapIds.map(rid => {
        const rm = (typeof findRoadmap === 'function') ? findRoadmap(rid) : null;
        const nm = rm ? (rm.name || '(unnamed)') : '(missing roadmap)';
        return '<span class="ff-link-chip" data-rmid="' + escapeHtml(rid) + '" style="display:inline-flex; align-items:center; gap:4px; padding:2px 6px 2px 8px; background:#fef3c7; color:#92400e; border-radius:12px; font-size:11.5px">' +
          escapeHtml(nm.slice(0, 50)) + (nm.length > 50 ? '…' : '') +
          '<button type="button" class="ff-link-x" data-rm-rmid="' + escapeHtml(rid) + '" style="background:transparent; border:none; cursor:pointer; color:#92400e; font-size:14px; padding:0 2px; line-height:1">×</button>' +
        '</span>';
      }).join('');
      rCont.querySelectorAll('.ff-link-x').forEach(btn => {
        btn.addEventListener('click', () => {
          _ffLinkedRoadmapIds = _ffLinkedRoadmapIds.filter(x => x !== btn.dataset.rmRmid);
          _ffRenderLinkChips();
        });
      });
    }
...
// Search inputs: filter the list of tasks/roadmaps and show top matches
// below. Click a row → add to the selection chips, clear the search.
function _ffWireLinkSearches() {
  const tInp = document.getElementById('ff_linkTasksSearch');
  const tRes = document.getElementById('ff_linkTasksResults');
  const rInp = document.getElementById('ff_linkRmSearch');
  const rRes = document.getElementById('ff_linkRmResults');
  if (tInp && tRes && !tInp.__bnWired) {
    tInp.__bnWired = true;
    const renderT = () => {
      const q = (tInp.value || '').trim().toLowerCase();
      if (!q) { tRes.style.display = 'none'; tRes.innerHTML = ''; return; }
      const matches = (STORE.tasks || [])
        .filter(t => !_ffLinkedTaskIds.includes(t.id) && (t.subject || '').toLowerCase().includes(q))
        .slice(0, 8);
      if (!matches.length) {
        tRes.innerHTML = '<div style="padding:8px; color:#9a9a9a; font-size:12px">No matches</div>';
      } else {
        tRes.innerHTML = matches.map(t =>
          '<div class="ff-link-row" data-tid="' + escapeHtml(t.id) + '" style="padding:6px 10px; cursor:pointer; font-size:13px; border-bottom:1px solid #f3f1ed">' +
            escapeHtml((t.subject || '(no title)').slice(0, 60)) +
          '</div>'
        ).join('');
        tRes.querySelectorAll('.ff-link-row').forEach(row => {
          row.addEventListener('mouseenter', () => row.style.background = '#f5f3ee');
          row.addEventListener('mouseleave', () => row.style.background = '');
          row.addEventListener('click', () => {
            const tid = row.dataset.tid;
            if (tid && !_ffLinkedTaskIds.includes(tid)) _ffLinkedTaskIds.push(tid);
            tInp.value = '';
    ...
    tInp.addEventListener('input', renderT);
    tInp.addEventListener('focus', renderT);
    tInp.addEventListener('blur', () => setTimeout(() => { tRes.style.display = 'none'; }, 180));
  }
  if (rInp && rRes && !rInp.__bnWired) {
    rInp.__bnWired = true;
    const renderR = () => {
      const q = (rInp.value || '').trim().toLowerCase();
      const all = (typeof getRoadmaps === 'function') ? getRoadmaps() : ((STORE && STORE.roadmaps) || []);
      const matches = all
        .filter(r => !_ffLinkedRoadmapIds.includes(r.id) && (!q || (r.name || '').toLowerCase().includes(q)))
        .slice(0, 8);
      if (!matches.length) {
        rRes.innerHTML = '<div style="padding:8px; color:#9a9a9a; font-size:12px">No matches</div>';
      } else {
        rRes.innerHTML = matches.map(r =>
          '<div class="ff-link-row" data-rmid="' + escapeHtml(r.id) + '" style="padding:6px 10px; cursor:pointer; font-size:13px; border-bottom:1px solid #f3f1ed">' +
            escapeHtml((r.name || '(unnamed)').slice(0, 60)) +
          '</div>'
        ).join('');
        rRes.querySelectorAll('.ff-link-row').forEach(row => {
          row.addEventListener('mouseenter', () => row.style.background = '#f5f3ee');
          row.addEventListener('mouseleave', () => row.style.background = '');
          row.addEventListener('click', () => {
            const rid = row.dataset.rmid;
            if (rid && !_ffLinkedRoadmapIds.includes(rid)) _ffLinkedRoadmapIds.push(rid);
            rInp.value = '';
    ...
    rInp.addEventListener('input', renderR);
    rInp.addEventListener('focus', renderR);
    rInp.addEventListener('blur', () => setTimeout(() => { rRes.style.display = 'none'; }, 180));
  }
}
// Wire once on script load (idempotent — _bnWired guards against re-binding).
_ffWireLinkSearches();
...
document.getElementById('ff_saveBtn').addEventListener('click', () => {
  ...
  if (filesEditingId) {
    ...
    if (f) {
      ...
      // Persist the in-modal task/roadmap links (might be empty arrays if
      // the user cleared everything — that's fine, we still write them).
      f.taskIds = _ffLinkedTaskIds.slice();
      f.roadmapIds = _ffLinkedRoadmapIds.slice();
    }
  } else {
    ...
    STORE.driveFiles.unshift({
      ...
      addedAt: Date.now(),
      addedBy: 'U07E6NHMLBV',
      taskIds: _ffLinkedTaskIds.slice(),
      roadmapIds: _ffLinkedRoadmapIds.slice()
    });
  }
  saveStore(STORE);
  closeFileModal();
...
// Open the Drive Picker with a generic `onPicked(doc)` callback. The Files-page
// flow passes a callback that fills the form fields; the task modal flow passes
// one that creates / links a STORE.driveFiles entry directly.
function bnOpenGoogleDrivePickerWith(onPicked, opts) {
  const status = (opts && opts.statusEl) || document.getElementById('ff_pickerStatus');
      ...
      .setCallback((data) => {
        if (data.action === google.picker.Action.PICKED && data.docs && data.docs.length > 0) {
          ...
          if (typeof onPicked === 'function') {
            try { onPicked({ url, name: d.name || '', raw: d }); } catch (e) { console.warn('picker callback error', e); }
          }
        }
      })
      .build();
...
window.bnOpenGoogleDrivePickerWith = bnOpenGoogleDrivePickerWith;

// Legacy entry point used by the Files-page "+ Pick from Drive" button. Wraps
// the generic picker with a callback that fills the file-form fields, as
// before. Kept as a named function so the existing bind below still works.
function bnOpenGoogleDrivePicker() {
  bnOpenGoogleDrivePickerWith((doc) => {
    document.getElementById('ff_url').value = doc.url;
    if (!document.getElementById('ff_name').value.trim()) {
      document.getElementById('ff_name').value = doc.name || '';
    }
    if (typeof ffUpdateTypePreview === 'function') ffUpdateTypePreview();
  });
}

// Picker invoked from the task modal: pick a Drive file and link it to the
// task in one shot. If the picked URL already exists in STORE.driveFiles,
// just add the task id to that file's taskIds[]. Otherwise create a new file
// entry with `sharedWith='team'` so it's visible to all teammates by default,
// and call onAdded so the modal re-renders its linked-files chip list.
function bnPickDriveFileForTask(taskId, onAdded) {
  if (!taskId) return;
  bnOpenGoogleDrivePickerWith((doc) => {
    if (!doc || !doc.url) return;
    if (!Array.isArray(STORE.driveFiles)) STORE.driveFiles = [];
    let f = STORE.driveFiles.find(x => x && x.url === doc.url);
    let touchedTeam = false;
    if (f) {
      if (!Array.isArray(f.taskIds)) f.taskIds = [];
      if (!f.taskIds.includes(taskId)) {
        f.taskIds.push(taskId);
        f.updatedAt = Date.now();
        if (f.sharedWith === 'team') touchedTeam = true;
      }
    } else {
      f = {
        id: 'f' + Math.random().toString(36).slice(2, 10),
        ...
        sharedWith: 'team',
        type: (typeof detectFileType === 'function') ? detectFileType(doc.url) : '',
        addedAt: Date.now(),
        taskIds: [taskId],
        roadmapIds: []
      };
      STORE.driveFiles.unshift(f);
      touchedTeam = true;
    }
    try { saveStore(STORE); } catch (_) {}
    if (typeof bnUpdateFilesBadge === 'function') { try { bnUpdateFilesBadge(); } catch (_) {} }
    if (touchedTeam && typeof bnSyncTeamFilesAfterChange === 'function') {
      try { bnSyncTeamFilesAfterChange(); } catch (_) {}
    }
    if (typeof onAdded === 'function') {
      try { onAdded(f); } catch (_) {}
    }
  });
}
window.bnPickDriveFileForTask = bnPickDriveFileForTask;
