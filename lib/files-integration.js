...
function driveThumbnailUrl(url, size) {
...
const BN_FILES_REPO_NAME = 'berni-navigator';
...
async function bnLoadTeamFiles(opts) {
  ...
  try {
    ...
    if (!res.ok) {
      // No team-files.json yet — that's fine
...
function bnPromptForGhToken() {
  const t = prompt(
    'To sync Team files across devices, paste your GitHub Personal Access Token (fine-grained) with Contents: write on BooklineBerni/berni-navigator.\n\n' +
