...
async function bnAdminPreviewAs(email) {
  ...
  if (!email) {
    ...
    // (now non-preview) admin caller. Common STORE_KEY variants — clear all just in case.
    try { localStorage.removeItem('bookline-store-v1'); } catch (_) {}
    try { localStorage.removeItem('bookline-store'); } catch (_) {}
  ...
  // Hop to Home — the previewed user wouldn't see Admin anyway, and that's the page
  // we were on. Use the standard view-switch path.
  try {
