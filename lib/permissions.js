...
// lib/permissions.js
// ---------------------------------------------------------------------------
// Admin/permissions management UI for Team Navigator.
//
...
// Staged-change buffers for the Admin → Per-user permissions panel. Changing a
// dropdown stages the new value here instead of firing an RPC immediately; the
// admin clicks "Apply" in the section header to commit every pending change
// in one batch. Avoids the per-row confirm() popup that interrupted the flow.
//
//   bnAdminPermsPending      → Map<user_id, new_permission>  (signed-in rows)
//   bnAdminAllowPermsPending → Map<email,   new_permission>  (allowlist rows)
const bnAdminPermsPending      = new Map();
const bnAdminAllowPermsPending = new Map();
...
function bnUpdateAdminNavVisibility() {
  ...
  //   • Name (Team Navigator) → previewed user's name, in their signature colour
  ...
  if (brand) {
    ...
    // and signed-in non-admins always see the plain "B / Team Navigator".
    if (inPreview && isAdmin && bnPreviewAsInfo) {
      ...
      // Name: replace "Team Navigator" with the previewed user's name, in their colour.
      if (nameEl) {
      ...
      // Restore default brand (cat GIF logo).
      brand.style.cursor = '';
      brand.title = '';
      if (logo) {
        logo.innerHTML = '<img src="logo.gif" alt="" referrerpolicy="no-referrer" style="width:100%;height:100%;object-fit:cover;display:block;">';
        logo.style.background = '';
        logo.style.color = '';
      }
      if (nameEl) {
        nameEl.textContent = 'Team Navigator';
        nameEl.style.color = '';
      }
...
async function bnAdminAddAllowlist(email, notes, permission) {
  ...
  const perm = ['none','restricted_view','member','admin'].includes(permission) ? permission : 'restricted_view';
  try {
...
async function bnPopulateProfilePermsPanel() {
  ...
  const opts = ['none','restricted_view','member','admin'].map(p => `<option value="${p}" ${currentPerm === p ? 'selected' : ''}>${p}</option>`).join('');
...
async function bnRenderAdminPage() {
  ...
  if (allowEl) {
    // ALLOWLIST shows ONLY pending invitations — users that have been invited but
    // have NOT yet completed sign-in (i.e., no row in user_permissions). As soon
    // as someone signs in, they move to the Per-user permissions panel on the
    // right, and revocation happens from there.
    const signedInEmails = new Set(
      perms.map(p => (p.email || '').toLowerCase()).filter(Boolean)
    );
    const pendingOnly = (allowlist || []).filter(row => {
      const e = (row.email || '').toLowerCase();
      return e && !signedInEmails.has(e);
    });
    if (!pendingOnly.length) {
      allowEl.innerHTML = '<div style="padding:14px; color:#888; text-align:center;">No pending invitations. Everyone in the allowlist has signed in.</div>';
    } else {
      allowEl.innerHTML = pendingOnly.map(row => {
        const perm = row.permission || 'none';
        const opts = ['none','restricted_view','member','admin'].map(p => `<option value="${p}" ${perm === p ? 'selected' : ''}>${p}</option>`).join('');
        return `
        <div style="display:flex; align-items:center; gap:10px; padding:10px 14px; border-bottom:1px solid #f0f0f0;">
          <div style="flex:1;">
            ...
            <div style="font-size:11px; color:#d97706; margin-top:2px;">Pending sign in${row.notes ? ' · ' + escapeHtml(row.notes) : ''}</div>
          </div>
          <select data-bn-allow-perm="${escapeHtml(row.email)}" data-bn-allow-perm-orig="${escapeHtml(perm)}" title="Permission applied when the user signs in. Click Apply (top-right of Per-user permissions) to save." style="padding:6px 8px; border:1px solid #d0d0d0; border-radius:6px;">${opts}</select>
      ...
      // Allowlist permission dropdowns: stage instead of immediately calling the RPC.
      // The Apply button at the top-right of Per-user permissions also commits these.
      allowEl.querySelectorAll('[data-bn-allow-perm]').forEach(sel => {
        sel.addEventListener('change', () => {
          const email = sel.getAttribute('data-bn-allow-perm');
          const orig  = sel.getAttribute('data-bn-allow-perm-orig');
          if (sel.value === orig) bnAdminAllowPermsPending.delete(email);
          else bnAdminAllowPermsPending.set(email, sel.value);
          // Visual cue: dotted outline + warm bg when dirty.
          if (sel.value === orig) { sel.style.outline=''; sel.style.background=''; }
          else { sel.style.outline='2px dashed #f59e0b'; sel.style.background='#fffbeb'; }
          bnUpdateAdminApplyButton();
        });
      });
    }
  }
  ...
  if (permsEl) {
    // PER-USER PERMISSIONS shows ONLY signed-in users (rows that exist in
    // user_permissions). Pending invitations live exclusively on the left
    // allowlist panel until the person signs in for the first time.
    const merged = [];
    perms.forEach(p => {
      if (!p.email) return;
    ...
    if (!merged.length) {
      permsEl.innerHTML = '<div style="padding:14px; color:#888; text-align:center;">No signed-in users yet. Invite people via the allowlist on the left.</div>';
    } else {
      ...
      permsEl.innerHTML = merged.map(row => {
        ...
        const opts = ['none','restricted_view','member','admin'].map(p => `<option value="${p}" ${row.permission === p ? 'selected' : ''}>${p}</option>`).join('');
        ...
        return `
          <div style="display:flex; align-items:center; gap:12px; padding:10px 14px; border-bottom:1px solid #f0f0f0;">
            ...
            <select data-bn-perm-user="${escapeHtml(row.user_id)}" data-bn-perm-user-orig="${escapeHtml(row.permission || '')}" ${isSelf ? 'disabled title="You can\'t change your own permission"' : 'title="Stage a permission change — Apply (top-right) commits it."'} style="padding:6px 8px; border:1px solid #d0d0d0; border-radius:6px;">${opts}</select>
            ...
            ${isSelf ? '' : `<button class="btn" data-bn-perm-revoke="${escapeHtml(row.user_id)}" data-bn-perm-email="${escapeHtml(row.email)}" type="button" style="color:#9b1c1c;" title="Revoke access — removes the user from both user_permissions and the allowlist">Revoke</button>`}
          </div>
        `;
      }).join('');
      ...
      // Per-user permission dropdowns: stage the new value into bnAdminPermsPending
      // instead of firing the RPC immediately. The Apply button in the section header
      // commits every staged change in one batch (no per-row confirm popup).
      permsEl.querySelectorAll('[data-bn-perm-user]').forEach(sel => {
        sel.addEventListener('change', () => {
          const uid  = sel.getAttribute('data-bn-perm-user');
          const orig = sel.getAttribute('data-bn-perm-user-orig');
          if (sel.value === orig) bnAdminPermsPending.delete(uid);
          else bnAdminPermsPending.set(uid, sel.value);
          if (sel.value === orig) { sel.style.outline=''; sel.style.background=''; }
          else { sel.style.outline='2px dashed #f59e0b'; sel.style.background='#fffbeb'; }
          bnUpdateAdminApplyButton();
        });
      ...
      // Revoke access: drops the user_permissions row AND the allowlist row in one
      // RPC call. The button replaces the "Remove" affordance that used to live on
      // the signed-in user's allowlist row.
      permsEl.querySelectorAll('[data-bn-perm-revoke]').forEach(b => {
        b.addEventListener('click', async () => {
          const uid   = b.getAttribute('data-bn-perm-revoke');
          const email = b.getAttribute('data-bn-perm-email');
          if (!confirm('Revoke access for ' + email + '?\n\nThey will be removed from the per-user permissions AND the signup allowlist, and will not be able to sign in again until you re-invite them.')) return;
          try {
            const { error } = await bnSupabase.rpc('admin_revoke_user_access', { target_user_id: uid });
            if (error) { alert('Error: ' + error.message); return; }
          } catch (e) { alert('Error: ' + (e && e.message || e)); return; }
          if (typeof bnRenderAdminPage === 'function') bnRenderAdminPage();
        });
      });
    }
  }
  // Reflect the current pending-change counts on the Apply button after render.
  bnUpdateAdminApplyButton();
}
// Updates the visibility + label of the Apply button at the top of Per-user
// permissions. Shows it only when there's at least one staged change across
// both buffers (allowlist + per-user). Label includes the count.
function bnUpdateAdminApplyButton() {
  const btn   = document.getElementById('bnAdminPermsApplyBtn');
  const count = document.getElementById('bnAdminPermsApplyCount');
  if (!btn) return;
  const n = bnAdminPermsPending.size + bnAdminAllowPermsPending.size;
  btn.style.display = n > 0 ? '' : 'none';
  if (count) count.textContent = n > 0 ? ('(' + n + ')') : '';
}
// Commits every staged permission change in one batch. Goes through the two
// pending Maps, calls the matching RPC for each, surfaces aggregate errors,
// then re-renders the Admin page so the dropdowns are reset to clean.
async function bnApplyAdminPermChanges() {
  const btn = document.getElementById('bnAdminPermsApplyBtn');
  if (btn) { btn.disabled = true; const _t = btn.textContent; btn.textContent = 'Applying…'; btn.dataset._t = _t; }
  const errors = [];
  for (const [uid, perm] of bnAdminPermsPending) {
    try {
      const { error } = await bnSupabase.rpc('admin_grant_permission', { target_user_id: uid, new_permission: perm });
      if (error) errors.push('user ' + uid + ': ' + error.message);
    } catch (e) { errors.push('user ' + uid + ': ' + (e && e.message || e)); }
  }
  for (const [email, perm] of bnAdminAllowPermsPending) {
    try {
      const { error } = await bnSupabase.rpc('admin_set_allowlist_permission', { target_email: email, new_permission: perm });
      if (error) errors.push('allow ' + email + ': ' + error.message);
    } catch (e) { errors.push('allow ' + email + ': ' + (e && e.message || e)); }
  }
  bnAdminPermsPending.clear();
  bnAdminAllowPermsPending.clear();
  if (btn) { btn.disabled = false; btn.textContent = btn.dataset._t || 'Apply'; }
...
document.addEventListener('DOMContentLoaded', () => {
  ...
  // Wire the staged-Apply button at the top of Per-user permissions.
  const applyBtn = document.getElementById('bnAdminPermsApplyBtn');
  if (applyBtn) applyBtn.addEventListener('click', () => bnApplyAdminPermChanges());
});
