...
async function bnAdminPreviewAs(email) {
  ...
  // Exiting preview: explicitly clear the read-only flag and body class BEFORE the
  // cloud pull. If we don't, any stale state (or an `_filtered` flag from the cloud
  // RPC) could keep the admin locked into read-only mode after coming back to their
  // own view — exactly the bug the user reported.
  if (!bnPreviewAsEmail) {
    window.__bnReadOnly = false;
    try { document.body.classList.remove('bn-read-only'); } catch (_) {}
    // Re-fetch the admin's permission so it's freshly 'admin' and bnFetchPermission's
    // hard override for the owner email kicks in if anything was clobbered.
    if (typeof bnFetchPermission === 'function') {
      try { await bnFetchPermission(); } catch (_) {}
    }
  }
