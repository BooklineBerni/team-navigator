    ...
    if (inPreview && bnPreviewAsInfo) {
      ...
      if (photo) {
        if (bnPreviewAsInfo.photo) {
          ...
          photo.alt = bnPreviewAsInfo.name || '';
        } else {
          // No photo → use the person's signature colour as the avatar background.
      ...
      if (nameEl) {
        ...
        // Each person has a signature colour in DEFAULT_TEAM / EXTERNAL_TEAM — use it
        // so the previewed user's name matches how they look on the People page.
        nameEl.style.color = bnPreviewAsInfo.color || '#94a3b8';
      }
