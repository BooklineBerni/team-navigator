...
function bnUpdateAdminNavVisibility() {
  ...
  // Brand B becomes the way back to the admin's own view while previewing.
  const brand = document.getElementById('bnBrand');
  const logo = document.getElementById('bnBrandLogo');
  if (brand) {
    if (inPreview) {
      brand.style.cursor = 'pointer';
      brand.title = 'Back to my admin view';
      ...
      brand.style.cursor = '';
      brand.title = '';
      if (logo) logo.textContent = 'B';
    }
  }
  // Render the preview-as card under the brand.
  const card = document.getElementById('bnPreviewAsCard');
  if (card) {
    if (inPreview && bnPreviewAsInfo) {
      const photo = document.getElementById('bnPreviewAsPhoto');
      const nameEl = document.getElementById('bnPreviewAsName');
      const emailEl = document.getElementById('bnPreviewAsEmail');
      const permEl = document.getElementById('bnPreviewAsPerm');
      if (photo) {
        if (bnPreviewAsInfo.photo) {
          photo.src = bnPreviewAsInfo.photo;
          photo.style.display = '';
          photo.style.background = '';
          photo.textContent = '';
        } else {
          // Fallback: coloured initials in place of a photo.
          photo.removeAttribute('src');
          photo.style.display = 'flex';
          photo.style.background = bnPreviewAsInfo.color || '#9ca3af';
          photo.style.alignItems = 'center';
          photo.style.justifyContent = 'center';
          photo.style.color = '#fff';
          photo.style.fontWeight = '600';
          // Initials via the alt attribute won't render; use a sibling text node instead.
          photo.alt = bnPreviewAsInfo.initials || '?';
        }
      }
      if (nameEl) nameEl.textContent = bnPreviewAsInfo.name || bnPreviewAsInfo.email;
      if (emailEl) emailEl.textContent = bnPreviewAsInfo.email || '';
      if (permEl) permEl.textContent = bnPreviewAsInfo.permission || 'unknown';
      card.style.display = '';
    } else {
...
async function bnAdminPreviewAs(email) {
  const wasInPreview = !!bnPreviewAsEmail;
  bnPreviewAsEmail = email || null;
  // Resolve preview-as info (photo, name, permission) so the sidebar card can render
  // BEFORE we pull from the cloud (so the card is up immediately, even though the
  // store will refresh asynchronously).
  bnPreviewAsInfo = bnPreviewAsEmail ? await bnResolvePreviewInfo(bnPreviewAsEmail) : null;
  ...
  try {
    if (typeof currentView !== 'undefined') {
      if (bnPreviewAsEmail) {
        // Entering preview: hop to Home so the user lands somewhere the previewed
        // user actually sees. (They'd never see Admin themselves.)
        currentView = 'home';
        if (typeof render === 'function') render();
      } else if (wasInPreview) {
        // Just exited preview: take the admin back to where they started.
        currentView = 'admin';
        if (typeof render === 'function') render();
      }
    }
  } catch (_) {}
  if (typeof bnRenderAdminPage === 'function' && !bnPreviewAsEmail) bnRenderAdminPage();
}
