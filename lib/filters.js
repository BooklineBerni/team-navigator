...
(function () {
  ...
  // Global visibility rule for UNASSIGNED tasks (no responsibleId):
  //   - Admins (NOT in preview-as) see all unassigned tasks.
  //   - Everyone else (members, restricted_view, admins in preview-as) only sees
  //     unassigned tasks they themselves proposed.
  // Applies to EVERY view (List, by-person, profile, dashboard counts, etc.)
  // because all of them go through `allFiltersOK` via `taskMatchesFilters`.
  // Tasks with a responsibleId are unaffected — this rule only narrows unassigned.
  function unassignedVisibilityOK(t, s) {
    ...
    if (!myPid) return false;              // not logged in / no person id → hide
