...
(function () {
  ...
  //                  not even admins-in-preview, not even proposed-by users).
  //   • 'Team'     → responsible + every proposed-by user sees it. Outsiders
  //                  (people not in either list) don't.
  //   • 'Everyone' / empty / anything else → no restriction, visible to all.
  // Admin (NOT in preview-as) sees everything regardless — they need a clear
  // god-view to manage the workspace. Admins in preview-as honor the rule
  // because the preview is supposed to mirror what the previewed user sees.
  function shareWithVisibilityOK(t, s) {
    const sw = t && t.shareWith;
    if (!sw || sw === 'Everyone') return true;
    if (s && s.isAdminLive) return true;
